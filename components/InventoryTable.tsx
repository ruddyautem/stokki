...
const InventoryTable = ({ items, onDeleteProduct }: InventoryTableProps) => {
  ...
  const Checkbox = ({
    ...
    <button
      type="button"
      onClick={onChange}
      className={`relative w-5 h-5 border-2 rounded-md transition-all duration-200 flex items-center justify-center ${checked ? "bg-slate-800 border-slate-800" : "bg-white border-slate-300"} hover:border-slate-400 cursor-pointer`}
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <tr
                  key={item.id}
                  className={`hover:bg-slate-50 transition-colors ${selectedIds.includes(item.id) ? "bg-slate-100" : ""}`}
                >
                  ...
                  <td className="px-6 py-4 text-sm">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getQuantityBadge(item)}`}
                    >
                      {item.quantity}
                    </span>
                  </td>
        ...
        {selectedIds.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex justify-between items-center">
            ...
            <button
              ...
              disabled={isDeleting}
              className="flex items-center gap-2 px-5 py-2.5 bg-slate-800 text-white rounded-lg hover:bg-slate-900 transition-all font-medium shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            >
      ...
      <div className="lg:hidden space-y-3">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div
              key={item.id}
              className={`bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden transition-colors ${selectedIds.includes(item.id) ? "ring-2 ring-slate-800" : ""}`}
            >
              ...
              <div className="flex items-start justify-between p-4 border-b border-slate-100">
                ...
                <button
                  type="button"
                  onClick={() => handleSingleDeleteClick(item.id)}
                  className="p-2 hover:text-red-500 text-red-600 hover:bg-red-100 rounded-lg transition-all cursor-pointer shrink-0 ml-2"
                  title="Supprimer"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
              ...
              <div className="grid grid-cols-3 gap-px bg-slate-100">
                ...
                <div className="bg-white p-3 flex flex-col items-center text-center">
                  ...
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-0.5 ${getQuantityBadge(item)}`}
                  >
                    {item.quantity}
                  </span>
                </div>
              ...
              {item.lowStockAt !== null && (
                <div className="bg-white p-3 flex items-center justify-center border-t border-slate-100">
                  <span className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold mr-2">
                    Alerte à:
                  </span>
                  <span className="text-xs font-medium text-slate-600">
                    {item.lowStockAt} unités
                  </span>
                </div>
              )}
            </div>
        ...
        {selectedIds.length > 0 && (
          <div className="fixed bottom-4 left-4 right-4 z-30 bg-slate-800 text-white rounded-xl shadow-2xl p-4 flex items-center justify-between">
            ...
            <button
              ...
              onClick={handleBulkDeleteClick}
              disabled={isDeleting}
              className="flex items-center gap-2 px-4 py-2 bg-red-600 rounded-lg hover:bg-red-700 transition-all font-medium text-sm disabled:opacity-50 cursor-pointer"
            >
